...
var dBounce,wBounce,derezz, music;
...
function setup() {
  ...
  derezz = createAudio('drezz.mp3');
  dBounce = createAudio('dBounce.mp3');
  wBounce = createAudio('bounce.mp3');
...
function draw() {
  ...
  if(gameState=="stand-by"){
    ...
    if(mousePressedOver(play) && menu_text==false){
  ...
  if(disc1.isTouching(disc2)||disc1.isTouching(disc3)||disc2.isTouching(disc1)||disc2.isTouching(disc3)||disc3.isTouching(disc1)||disc3.isTouching(disc1)){
    dBounce.volume(0.2);
    dBounce.play();
  }
  ...
  if(gameState!="play"){
    ...
    fill("white");
    textSize(18);
    text("For controls and tips, click here", arrow1.x-190,arrow1.y-35);
    ...
    fill("black");
    text("The objective is to use your disc or discs to derezz the other",350,350);
    text("program, this game requires two physical players.",380,375);

    fill("cyan");
    text("Control the cyan program with the arrow keys, launch your disc",350,425);
    text("with the Enter key. Your victories are on the right top side", 380,450);

    fill("orange");
    text("Control the orange program with WASD, launch your discs",350,500);
    text("with Q and E. Your victories are on the left top side", 380,525);

    fill("red");
    text("After each round, you will have to retrive your discs",400,575);
    text("from the place they spawn in", 480,600);
